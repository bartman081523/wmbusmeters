...
> agent (or human contributor) needs to work in this repository: the
> specification landscape (EN 13757 / OMS / libmbus), build commands,
> coding conventions, driver development workflow, XMQ format, formula
> system, library fields, architecture, testing, CI, runtime, and
> dependencies. It is **loaded on demand by the metaprompt in
> [`AGENTS.md`](../../AGENTS.md)** — together with
> [`research.md`](research.md) (methodology) and
...
- **Core (C++).** `src/` — hardware layer (`wmbus_*.cc`), protocol parsing
  (`wmbus.cc`, `dvparser.cc`), driver registry and dynamic XMQ loader
  (`drivers.cc`, `driver_dynamic.cc`), formula engine (`formula.cc`),
  printer (`printer.cc`), units (`units.cc`).
- **Drivers (declarative).** `drivers/src/*.xmq` — one XMQ file per meter
  type. No new C++ drivers are accepted; see "Driver Development" above.
- **C++ unit tests.** `src/testinternals.cc` — plain `void test_<name>()`
  functions registered in the `LIST_OF_TESTS` X-macro; run via
  `build/testinternals <substring>`.
- **Shell tests.** `tests/` — ~60 scripts covering protocol variants, config
  parsing, formulas, AES, mbus, etc.; `tests/test_drivers.sh` runs all driver
  XMQ test cases.

## Generated artefacts (never hand-edit)

- `src/generated_database.cc` — re-embeds all drivers; regenerated by
  `cd drivers && make install`.
- `tests/generated_tests.xmq` — driver test catalogue; same regeneration step.

After editing any `drivers/src/*.xmq`, run `cd drivers && make install` and
verify with `git diff --quiet drivers` before committing.

## Test workflow (project-specific)

For decoding logic, write a failing test first, then implement until it
passes. Two locations:

- **C++ unit test.** Add `X(my_test)` to `LIST_OF_TESTS` in
  `src/testinternals.cc`, implement `void test_my_test()` using `assert()`
  and `printf("ERROR! ...")` for failure reporting. Run with
  `make && build/testinternals my_test` (substring match).
- **Driver test.** Add a `<test>` block to the relevant `drivers/src/<name>.xmq`
  with a raw telegram hex, expected JSON (sorted via `jq --sort-keys`), and
  expected CSV fields. Run with
  `cd drivers && make testonly DRIVER=<name>`.

For driver tests, there is no automated way to capture new expected output —
update `json = '...'` and `fields = '...'` by hand after verifying the
binary's output is correct.

## Units table reference

Units are enumerated and controlled. The authoritative table is
`LIST_OF_UNITS` in `src/units.h`; the binary command `wmbusmeters --listunits`
prints it. Add a real unit to that table rather than synthesising or
concatenating unit strings on the fly.

## Issue templates

Bug reports, enhancements and device-support requests use the templates in
`.github/ISSUE_TEMPLATE/` — pick the matching one:

- `driver-request.yml` for new or improved meter support.
- `wmbusmeters-request.yml` for the rest.

## Cross-checking decoding

For M-Bus data-record decoding (DIF/VIF, compact profiles, date/time types)
libmbus is a useful comparison: see `mbus/mbus-protocol.c` and its VIF/VIFE
tables. wmbusmeters implements *more* of the standard than libmbus (notably
the wireless parts — compact frames with hash signatures, the ELL header),
so libmbus is a **comparison, not a reference**. When they differ, the
standards (not libmbus) decide.
...
the standard, not invented — see [`research.md`](research.md).
